...
const { isLoggedIn, areUserDataFetched, isLoading, fetchUser, login, logout } = useAuth();
...
const toastPt = {
  ...
  closeButton: {
    class:
      'ml-auto p-1 rounded text-dual-grey-400 hover:text-dual-grey-700 bg-transparent border-none cursor-pointer shrink-0',
  },
      ...
      >
        ...
        <div class="flex items-center gap-3">
          <template v-if="isLoggedIn">
            <CommonButton class="mr-4" @click="navigateTo('/create-recipe')">Створити рецепт</CommonButton>
            <button
